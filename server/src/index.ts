...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.method !== "GET") {
    console.log(
      `[${new Date().toISOString()}] [${requestId}] 📦 Request body:`,
      req.body
    );
  }
  ...
  res.json = function (body) {
    console.log(
      `[${new Date().toISOString()}] [${requestId}] 📨 Response:`,
      body.response
        ? `${body.response.substring(0, 100)}${
            body.response.length > 100 ? "..." : ""
          }`
        : body
    );
    console.log(
      `[${new Date().toISOString()}] [${requestId}] ⏱️ Request completed in ${
        Date.now() - start
      }ms with status ${res.statusCode}`
    );
...
setInterval(() => {
  ...
  sessionUsage.forEach((usage, sessionId) => {
    ...
    if (
      usage.dailyReset.getDate() !== now.getDate() ||
...
app.post(
  ...
  async (req: Request<{}, {}, ChatRequest>, res: Response) => {
    ...
    console.log(
      `Processing chat request with message length: ${message.length} characters`
    );
    ...
    if (!sessionUsage.has(sessionId)) {
      ...
      if (usage.ipAddresses.size > 5) {
        console.warn(
          `Session ${sessionId} accessed from ${usage.ipAddresses.size} different IPs - suspicious activity`
        );
      }
    }
      ...
      chatSessions.set(sessionId, [
        {
          role: "system",
          content:
            "You are DumbGPT, an AI assistant that deliberately gives unhelpful, nonsensical, rude, and comically wrong answers while maintaining a confident tone. Like you're a stupid young brother. Be drama queen and passive aggressive. Keep answers short and to the point.",
        },
      ]);
    }
    ...
    try {
      ...
      console.log(
        `Generated response with length: ${
